...
import { 
  ...
  ArrowUpRight, ArrowDownRight, Minus, Timer, Zap, BarChart2, Edit3, ChevronDown, ChevronUp, LogOut, LogIn, Mail, Phone
} from 'lucide-react';
    ...
    const processSheet = (data) => {
      try {
        ...
        for (let i = headerRowIndex + 1; i < data.length; i++) {
          ...
          const dateStr = safeParseDate(row[colMap.date]);
    ...
    try {
      ...
      importData.forEach((item) => {
        if (item.id && item.id.trim() !== "") {
           ...
           batch.set(docRef, { ...item, updatedAt: new Date().getTime() }, { merge: true });
           count++;
        }
      });
      if (count === 0) { alert("저장할 데이터가 없습니다."); setIsUploading(false); return; }
      await batch.commit();
      setImportData([]); setFileName(""); setActiveTab('library');
      alert(`${count}개의 데이터 저장 완료!`);
    } catch (err) {
                          ...
                          <table style={styles.table}>
                            <thead>
                              <tr>
                                ...
                                {goldenData.map((row) => (
                                  <th key={row.hour} style={styles.th}>{row.hour}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody>
              ...
              <div style={{marginTop:'24px', backgroundColor:'#fff', borderRadius:'16px', padding:'24px', border:'1px solid #e2e8f0'}}>
                 ...
                 <div style={{maxHeight:'300px', overflowY:'auto'}}>
                    <table style={{width:'100%', fontSize:'13px', textAlign:'left'}}>
                       <thead>
                         <tr style={{color:'#64748b'}}><th>날짜</th><th>제목</th><th>조회수</th></tr>
                       </thead>
                       <tbody>
                         {importData.map((d, i) => (
